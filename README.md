...
- **DNS as a control plane.** A Pi-hole handles DNS for the internal segments, with **Unbound behind it doing full recursive resolution**: queries start at the DNS root servers and go to each domain's own nameservers, so no single public resolver is handed the household's full lookup history. That claim has limits. Recursive DNS is unencrypted, so the ISP can still see the queries on the wire. And the guest and DMZ networks use a public resolver on purpose, so they never learn internal names. Availability comes from a second identical Pi-hole and Unbound box, not from a public fallback upstream (the config example explains why I stopped doing that). Pi-hole's blocklists filter ads, trackers, and known-malicious domains *for every device at once*, including the IoT gear that offers no settings of its own. Config example in [`pihole/unbound.example.conf`](pihole/unbound.example.conf).
...
- **A separate admin plane.** I do administration (SSH, hypervisor UI) over a **Tailscale mesh VPN with identity-based access**. For the DMZ that is the only way in, because no inter-VLAN rule lets anything initiate into it. For Servers it is a habit the firewall does not enforce: Trusted → Servers currently allows every port, so a laptop on Trusted can reach SSH and the hypervisor UI directly. Network gear is managed from Trusted over an explicit allow on 443 and 22. This matters because of the key decision below.
...
Every arrow that *isn't* drawn is the point: IoT cannot reach Servers, Guest cannot reach anything internal, nothing internal can initiate into the DMZ, and the DMZ can initiate into nothing internal.
...
**The decision: default-deny between all segments, including against myself. The admin path into the DMZ runs over an identity-based VPN, not over firewall exceptions.**
...
**2. Segmentation you exempt yourself from isn't segmentation.** The DMZ is the segment most likely to be compromised; it's the one taking traffic from the internet. The single most valuable rule in the whole set is *"nothing initiates into the DMZ, the DMZ initiates into nothing."* An allow rule from the server segment into the DMZ "just for admin" would not have let the DMZ open connections back, since the firewall is stateful and only replies return. What it would have done is make a subnet address the credential: anything on the Servers VLAN, including a service I had stopped thinking about, could open a session to the most exposed hosts I run. Instead, admin access rides the Tailscale mesh, authenticated by identity and device rather than by which subnet the packet came from. An admin session into a compromised host is exposed to that host whichever way it arrived, so the mesh path needs containment of its own; the policy doc covers that.
...
**4. DNS is a boundary, not just a service.** Running Unbound as a full recursive resolver means no single resolver operator is handed the household's whole DNS history (it is not encryption, and the ISP can still watch queries go by), and Pi-hole gives one enforcement point for every device, including the lightbulbs that will never get a settings page. Keeping the DMZ *off* internal DNS is the same thinking in reverse: name resolution is reconnaissance, and a compromised public host doesn't get to do reconnaissance on my internal namespace.

**The cost of this decision, so it's on the record:** this is more moving parts and more ways to confuse myself. A misfiled device gets the wrong VLAN and "the internet is broken," and at least once the firewall was the "bug" I spent an evening debugging. It is also not as strict everywhere as the headline suggests: Trusted → Servers is a blanket allow, and the rules shown are IPv4 rules between VLANs, so traffic to the gateway itself and IPv6 need their own policy. Both are listed in the policy doc's limits. Self-hosted DNS also means DNS is now *my* uptime problem. For a network I administer alone, the containment is worth the friction. For a household of non-technical users with no admin on call, I'd simplify (fewer segments, more managed components). Knowing where that line sits matters more than the segmentation itself.
